@extends('layouts.template')
@section('content')
        <!-- Header Start -->
        <header id="steering-committee-section-1" class="overflow-x-hidden">  
            <div class="header-menu h-44 md:h-60 w-full bg-blue-400 bg-cover bg-bottom md:bg-center pt-2"
                style="background-image: url({{ asset('dist/img/bg-page-header.png') }})">
                <div data-aos="fade-up" class="container px-2 flex flex-col gap-0justify-center pt-20 md:pt-28">
                    <div class="header-title">
                        <h1 class="text-white text-2xl md:text-5xl">Steering Committee</h1>
                    </div>
                    <div class="text-xs md:text-base breadcrumbs text-white">
                        <ul>
                            <li><a href="./../../index.html">Home</a></li>
                            <li>Event</li>
                            <li>Steering Committee</li>
                        </ul>
                    </div>
                </div>
            </div>
        </header>
        <!-- Header End -->
    
        <!-- Section 2 Start -->
        <section id="steering-committee-section-2">
            <div class="container px-2 lg:px-52 py-10 md:py-16 flex flex-col gap-10">
    
                <!-- 1st -->
                <div data-aos="fade-up" class="content-1 flex flex-col gap-5">
                    <h1 class="text-center text-lg md:text-2xl font-medium">STEERING COMMITTEE OF INTERNATIONAL CONFERENCE
                        ON MEDICAL SCIENCE AND HEALTH (ICOMESH) AT HIGHER EDUCATION FOR TECHNOLOGY AND INNOVATION (HETI)
                        PROJECT
                        UNIVERSITY OF LAMPUNG
                    </h1>
                    <div class="overflow-x-auto">
                        <table class="table text-sm md:text-base">
                            <!-- head -->
                            <thead>
                                <tr class="text-mydark">
                                    <th>NO.</th>
                                    <th>NAME</th>
                                    <th>POSITION</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <th>1</th>
                                    <td>Prof. Dr. Ir. Lusmeilia Afriani, D.E.A., IPM</td>
                                    <td>Pengarah</td>
                                </tr>
                                <tr>
                                    <th>2</th>
                                    <td>Rudy, S.H., LL.M., LL.D.</td>
                                    <td>Anggota Pengarah</td>
                                </tr>
                                <tr>
                                    <th>3</th>
                                    <td>Dr. Anna Gustina Zainal, S.Sos., M.Si.</td>
                                    <td>Anggota  Pengarah</td>
                                </tr>
                                <tr>
                                    <th>4</th>
                                    <td>Dr. Ayi Ahadiat, S.E., M.B.A.</td>
                                    <td>Anggota  Pengarah</td>
                                </tr>
                                <tr>
                                    <th>5</th>
                                    <td>Dr. dr. Evi Kurniawaty, M.Sc</td>
                                    <td>Anggota  Pengarah</td>
                                </tr>
                                <tr>
                                    <th>6</th>
                                    <td>Prof. Dr. Jhon Hendri, M.S</td>
                                    <td>Anggota  Pengarah</td>
                                </tr>
                                <tr>
                                    <th>7</th>
                                    <td>Dr. Habibullah Jimad, S.E., M.M</td>
                                    <td>Anggota  Pengarah</td>
                                </tr>
                                <tr>
                                    <th>8</th>
                                    <td>Prof. Dr.  Satria Bangsawan,   S.E.,   M.Si</td>
                                    <td>Penanggung  Jawab</td>
                                </tr>
                                <tr>
                                    <th>9</th>
                                    <td>Ir. Andius  Dasa Putra, S.T.,  M.T., Ph.D., IPM</td>
                                    <td>Wakil Penanggung Jawab I</td>
                                </tr>
                                <tr>
                                    <th>10</th>
                                    <td>dr. Intanri Kurniati, Sp.PK</td>
                                    <td>Wakil Penanggung Jawab II</td>
                                </tr>
                                <tr>
                                    <th>11</th>
                                    <td>Dr. dr. Dian Isti Angraini, S. Ked., M.P.H.</td>
                                    <td>Wakil Penanggung Jawab III</td>
                                </tr>
                                <tr>
                                    <th>12</th>
                                    <td>Dr.si. dr. Syazili Mustofa, M.Biomed</td>
                                    <td>Ketua</td>
                                </tr>
                                <tr>
                                    <th>13</th>
                                    <td>Ramadhana Komala, S.Gz., M.Si</td>
                                    <td>Wakil Ketua I</td>
                                </tr>
                                <tr>
                                    <th>14</th>
                                    <td>Isroni Muhammad Miraj Mirza, SH, MH</td>
                                    <td>Wakil Ketua II</td>
                                </tr>
                                <tr>
                                    <th>15</th>
                                    <td>dr. Diana Mayasari, MKK</td>
                                    <td>Sekretaris</td>
                                </tr>
                                <tr>
                                    <th>16</th>
                                    <td>Suryadi Islami, S.Si., M. Biomed</td>
                                    <td>Divisi  Acara</td>
                                </tr>
                                <tr>
                                    <th>17</th>
                                    <td>Dr. dr. Khairunnisa, M.Kes AIFO</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>18</th>
                                    <td>dr. Agustyas Tjiptaningrum, SpPK</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>19</th>
                                    <td>dr. Novita Carolia, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>20</th>
                                    <td>dr. Hanna Mutiara, M.Kes, Sp.ParK</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>21</th>
                                    <td>Bayu Anggileo Pramesona, S.Kep, Ns., MMR, Ph.D</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>22</th>
                                    <td>Dr. Nuning Nurcahyani, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>23</th>
                                    <td>Apt. Dwi Aulia Ramdini, M.Farm</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>24</th>
                                    <td>dr. Anisa Nuraisa Djausal, MKM</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>25</th>
                                    <td>dr. Risti Graharti, M.Ling</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>26</th>
                                    <td>Ihsanti Dwi Rahayu, S.Farm., M.S.Farm</td>
                                    <td>Divisi Ilmia</td>
                                </tr>
                                <tr>
                                    <th>27</th>
                                    <td>Dr. dr. Jhons Fatriyadi Suwandi, M.Kes, Sp.ParK</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>28</th>
                                    <td>Dr. dr. Betta Kurniawan, M.Kes, Sp.ParK</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>29</th>
                                    <td>Dr. dr. Ety Apriliana, M.Biomed</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>30</th>
                                    <td>Dr. dr. Susianti, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>31</th>
                                    <td>Dr. dr. Indri Windarti, Sp.PA</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>32</th>
                                    <td>dr. Waluyo Rudiyanto, M.Kes</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>33</th>
                                    <td>Dr. dr. Reni Zuraida, M.Si</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>34</th>
                                    <td>Dr. Suharmanto, S.Kep, MKM</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>35</th>
                                    <td>Dr. Endah Setyaningrum, M.Biomed</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>36</th>
                                    <td>Bayu Putra Danan Jaya, S.Si, M.Si</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>37</th>
                                    <td>Apt. Citra Yuliyanda Pardilawati, S.Farm., M.Farm.</td>
                                    <td>Divisi Prosiding</td>
                                </tr>
                                <tr>
                                    <th>38</th>
                                    <td>Dr. dr. Fitria Saftarina, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>39</th>
                                    <td>dr. Nisa Karima, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>40</th>
                                    <td>dr. Tri Umiana Soleha, M.Kes</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>41</th>
                                    <td>Sutarto, SKM, M.Epid</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>42</th>
                                    <td>Andi Nafisah Tendri Adjeng, S.Farm, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>43</th>
                                    <td>Selvi Marcellia, S.Si., M.Sc</td>
                                    <td>Divisi Humas Publikasi</td>
                                </tr>
                                <tr>
                                    <th>44</th>
                                    <td>dr. Anggraeni Janar Wulan, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>45</th>
                                    <td>Afriyani, S.Farm, M.Farm</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>46</th>
                                    <td>dr. Giska Tri Putri, M.Ling</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>47</th>
                                    <td>dr. Maya Ganda Ratna, M.Sc</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>48</th>
                                    <td>Sofyan Musyabiq Wijaya, S.Gz, M.Gz</td>
                                    <td>Divisi Informasi & Teknologi</td>
                                </tr>
                                <tr>
                                    <th>49</th>
                                    <td>Hermansyah, S.Kom</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>50</th>
                                    <td>Nahrowi</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>51</th>
                                    <td>Erik Riyandi</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>52</th>
                                    <td>Aji Kurniawan Irawan</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>53</th>
                                    <td>Rendi</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>54</th>
                                    <td>Linda Septiani, S.Si., M.Sc.</td>
                                    <td>Divisi Sekretariatan</td>
                                </tr>
                                <tr>
                                    <th>55</th>
                                    <td>Dharmawanti, S.Sos</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>56</th>
                                    <td>Widya Wati, S.H., M.H</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>57</th>
                                    <td>M. Zaim Rozaan, S.Hub.In</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>58</th>
                                    <td>Ena, S.E</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>59</th>
                                    <td>Andri Okta Wijaya, S.H</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>60</th>
                                    <td>Apt. Ervina Damayanti, M.Clin, Pharm</td>
                                    <td>Divisi Liaison officer (LO)</td>
                                </tr>
                                <tr>
                                    <th>61</th>
                                    <td>dr. Putu Ristya Ayu S, M.Kes, Sp.PK (K)</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>62</th>
                                    <td>Apt. Zulpakor Oktoba, S.Si, M.Farm</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>63</th>
                                    <td>Femmy Andrifianie, S.Farm, M.Farm</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>64</th>
                                    <td>Endah</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>65</th>
                                    <td>Ramadhan Triyandi, S.Farm, M.Si</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>66</th>
                                    <td>M Iqbal</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>67</th>
                                    <td>Apt. M Fitra Wardhana Sayoeti, S.Farm, M.Farm</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>68</th>
                                    <td>Nuriah, A.Md</td>
                                    <td>Sie Konsumsi</td>
                                </tr>
                                <tr>
                                    <th>69</th>
                                    <td>Suharyani</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>70</th>
                                    <td>Agustina</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>71</th>
                                    <td>Vienda Redisti, Amd</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>72</th>
                                    <td>Romiani</td>
                                    <td>Anggota</td>
                                </tr>
                                <tr>
                                    <th>73</th>
                                    <td>Eka</td>
                                    <td>Anggota</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
    
    
            </div>
        </section>
        <!-- Section 2 End -->
@endsection