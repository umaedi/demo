        ...
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td>
                    ...
                    <button onclick="persence('{{ $tb->id }}', '1')" class="btn btn-success persence"><i class="fa fa-check"></i></button>
                    @else
                    <button onclick="persence('{{ $tb->id }}', '1')" class="btn btn-warning persence-1">Persence</button>
                    @endif
                    @if ($tb->status !== '0')
                        @if ($tb->status == '2')
                        <button onclick="persence('{{ $tb->id }}', '2')" class="btn btn-success"><i class="fa fa-check"></i></button>
                        @else
                        <button onclick="persence('{{ $tb->id }}', '2')" class="btn btn-warning persence-2">Persence</button>
                        @endif
                    @endif
                </td>
            </tr>
